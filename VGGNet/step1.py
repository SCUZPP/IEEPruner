...
def evaIndividual(ind, model, args, train_loader, test_loader, val_loader, original_filter):
    ...
    #计算微调后的准确率
    #acc, loss = train_forward(train_loader, test_loader, val_loader, model_new)  # test_forward(model_new)
    
    #print(acc, pruning_rate)
    #打印微调前的准确率和损失
    
    #print('step1微调后:  * accuracy {acc:.2f}, pruning {pruning:.2f}'
    #     .format(acc = acc, pruning=pruning_rate))
    
    #pruning_rate = 1 - np.sum(solution) / (sum(filter_nums))
    
    #print('step1微调前:  * accuracy {acc:.2f}, pruning {pruning:.2f}'
     #    .format(acc = acc, pruning=pruning_rate))
...
def get_population(model, args, train_loader, test_loader, val_loader):
    ...
    if args.depth == 16:  
        if args.rand:
            original_filter = pickle.load(open('filter_dict/filter_dict_vgg16_rand_fc.pkl', 'rb'))
            
            if not args.cuda:
                original_filter = original_filter[0 : 2]
            if args.constraint:
                target_dir = 'Results_vgg16_rand_fc_kg/'
                
            else:
                target_dir = 'Results_vgg16_rand_fc/'
        
        else:
            original_filter = pickle.load(open('filter_dict/filter_dict_vgg16.pkl', 'rb'))  
            if not args.cuda:
                original_filter = original_filter[0 : 2]
            target_dir = 'Results_vgg16/'

    elif args.depth == 19:
        if args.rand:
            original_filter = pickle.load(open('filter_dict/filter_dict_vgg19_rand_fc.pkl', 'rb'))
            if not args.cuda:
                original_filter = original_filter[0 : 2]
                
            if args.constraint:
                target_dir = 'Results_vgg19_rand_fc_kg/'
                
            else:
                target_dir = 'Results_vgg16_rand_fc/'

        else:
            original_filter = pickle.load(open('filter_dict/filter_dict_vgg19.pkl', 'rb'))  
            if not args.cuda:
                original_filter = original_filter[0 : 2]
            target_dir = 'Results_vgg19/'
    ...
    if args.depth == 16:
        if args.constraint:
            pickle.dump(population_init, open('population_dict/population_dict_vgg16_rand_fc_kg.pkl', 'wb'))
        
        else:
            if args.rand:
                pickle.dump(population_init, open('population_dict/population_dict_vgg16_rand_fc.pkl', 'wb'))  
            else:
                pickle.dump(population_init, open('population_dict/population_dict_vgg16.pkl', 'wb'))

    if args.depth == 19:
        if args.constraint:
            pickle.dump(population_init, open('population_dict/population_dict_vgg19_rand_fc_kg.pkl', 'wb'))
            
        else:
            if args.rand:
                pickle.dump(population_init, open('population_dict/population_dict_vgg19_rand_fc.pkl', 'wb'))  
            else:
                pickle.dump(population_init, open('population_dict/population_dict_vgg19.pkl', 'wb'))
                
                
    '''
    save_path = 'population_dict/population_dict_vgg%s_rand_%s_constraint_%s.pkl' % (args.depth, args.rand, args.constraint)
    print(save_path)
                                                                                 
    pickle.dump(population_init, open(save_path, 'wb'))'''
