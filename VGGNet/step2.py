...
def initialization(pop_size, gene_length, model, args, train_loader, test_loader, val_loader, original_filter, p0, target_dir):
    ...
    for i in range(pop_size):
        ...
        if args.retrain:
            with open(path_save + "population_retrain_300.pkl", 'wb') as f:
                pickle.dump(population, f)     
                
            with open(path_save + "population_retrain_30.pkl", 'wb') as f:
                pickle.dump(acc, f)     
        
        else:
            with open(path_save + "population_300.pkl", 'wb') as f:
                pickle.dump(population, f) 
                
            with open(path_save + "population_30.pkl", 'wb') as f:
                pickle.dump(acc, f) 
...
def get_population(model, args, train_loader, test_loader, val_loader):
    ...
    if args.depth == 16:
        if args.rand:
            ...
            if args.constraint:
                p0 = pickle.load(open('population_dict/population_dict_vgg16_rand_fc_kg.pkl', 'rb'))
                target_dir = 'Results2_vgg16_rand_fc_kg/'
                
            else:    
                p0 = pickle.load(open('population_dict/population_dict_vgg16_rand_fc.pkl', 'rb'))
                #存储结果
                target_dir = 'Results2_vgg16_rand_fc/'

        else:
            original_filter = pickle.load(open('filter_dict/filter_dict_vgg16.pkl', 'rb'))
            p0 = pickle.load(open('population_dict/population_dict_vgg16.pkl', 'rb'))
            target_dir = 'Results2_vgg16/'
        ...
        if args.rand:
            ...
            if args.constraint:
                p0 = pickle.load(open('population_dict/population_dict_vgg19_rand_fc_kg.pkl', 'rb'))
                target_dir = 'Results2_vgg19_rand_fc_kg/'
                
            else:    
                p0 = pickle.load(open('population_dict/population_dict_vgg19_rand_fc.pkl', 'rb'))
                #存储结果
                target_dir = 'Results2_vgg19_rand_fc/'


        else:
            original_filter = pickle.load(open('filter_dict/filter_dict_vgg19.pkl', 'rb'))
            p0 = pickle.load(open('population_dict/population_dict_vgg19.pkl', 'rb'))
            target_dir = 'Results2_vgg19/'
