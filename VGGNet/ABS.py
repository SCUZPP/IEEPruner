...
def absolute_weight_sum_conv(conv_name, para_data, res, num_t):
    ...
    res[conv_name[1]] = DRIVE(filter_list, num_t)

def absolute_weight_sum_fc(fc_name, para_data, res, num_t):
    
    fc_name = fc_name.split('.', 3)
    #if 'weight' in conv_name:
    p = copy.deepcopy(para_data)
    p = p.squeeze()
    filter_list = [p[i].squeeze().data.cpu().numpy() for i in range(p.size(0))]

    res[fc_name[1]] = DRIVE(filter_list, num_t)
...
def get_code(compress_rate, model, args, train_loader, test_loader, val_loader):
    ...
    for name, para in model.named_parameters():
        if para.requires_grad:
            if 'weight' in name:
                #如果是卷积层且参数维度为4
                if 'features' in name and para.dim() == 4:
                    num_t = int(para.data.shape[0] * compress_rate)
                    absolute_weight_sum_conv(name, para, res_ABS, num_t)

                elif 'classifier' in name and para.dim() == 2:
                    num_t = int(para.data.shape[0] * compress_rate)
                    absolute_weight_sum_fc(name, para, res_ABS, num_t)
